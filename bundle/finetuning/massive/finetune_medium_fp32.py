...
from models.model_medium_fp32 import GPTBatched, GPTClassification, GPTFlexAttention, XNLIDataset
...
def save_resume_checkpoint(model, optimizer, accelerator, global_step, epoch, batch_idx,
                           best_val_loss, running_loss, args):
...
def prepare_datasets(args, model):
    ...
    if args.train_datasets is not None and args.parent_dataset is not None and args.train_configs is not None:
        if args.eval_datasets is None:
            raise ValueError("Must provide --eval_datasets for MODE 5")
    ...
    if args.train_configs is not None:
        has_multi_eval = args.eval_parent_langs is not None
        if args.eval_config is None and not has_multi_eval:
        ...
        if args.from_disk:
            ...
            for config in args.train_configs:
                ...
                ds = _load_from_disk(str(local_path))
                train_datasets_list.append(ds["train"])
            ...
            if has_multi_eval:
                ...
                for lang in args.eval_parent_langs:
                    ...
                    ds = _load_from_disk(str(local_path))
                    val_datasets_list.append(ds["validation"])
                    print(f"  Loaded {lang} val (from disk): {len(ds['validation']):,} examples")
                validation_dataset = concatenate_datasets(val_datasets_list)
            else:
                local_path = pathlib.Path(args.parent_dataset) / args.eval_config
                eval_ds = _load_from_disk(str(local_path))
        ...
        if has_multi_eval:
            # Combine all eval languages into one val set
            val_datasets_list = []
            for lang in args.eval_parent_langs:
                ds = load_dataset(args.parent_dataset, lang, cache_dir=str(args.hf_cache))
                val_datasets_list.append(ds["validation"])
                print(f"  Loaded {lang} val: {len(ds['validation']):,} examples")
            validation_dataset = concatenate_datasets(val_datasets_list)
        else:
...
def prepare_multilang_eval(args, model):
    ...
    for lang, val_ds in eval_map.items():
        ...
        val_datasets[lang] = XNLIDataset(val_path, model.context_window)
...
def load_pretrained_model(args, model):
    """Load pretrained MEDIUM model from training script checkpoint and convert to batched version"""
    print(f"Loading pretrained MEDIUM model (16 layers, 8 heads, 1024 dim) from {args.pretrained_ckpt_path}")
    ...
    # DETECT vocab sizes from checkpoint - they're DIFFERENT!
    embed_vocab_size = state_dict['embed.weight'].shape[0]  # 50000 (not padded)
    lm_head_vocab_size = state_dict['lm_head_w'].shape[0]   # 50048 (padded)

    print(f"Detected from checkpoint:")
    print(f"  - Embed vocab size: {embed_vocab_size} (actual)")
    print(f"  - LM head vocab size: {lm_head_vocab_size} (padded)")
    print(f"  - Note: Only lm_head is padded, embeddings are not!")
    ...
    print(f"  Model: 16 layers, 8 heads, 1024 dimensions")
    print(f"  Vocab size: {embed_vocab_size} (from embeddings)")
...
def finetune(model, train_loader, val_loaders, optimizer, accelerator, best_val_loss, best_checkpoint_path, args,
    ...
    val_loaders: dict {lang: DataLoader} for per-language evaluation.
    best_val_loss: best average val loss across all languages.
    """
    global _SIGTERM_RECEIVED

    if args.wandb_log:
        config = {
            "model_size": "medium (16L, 8H, 1024D)",
        ...
        wandb.init(project=args.wandb_project, name=f"{args.dataset}-medium-{time.time()}", config=config)
        ...
        for batch_idx, (X, Y) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.eval_interval == 0:
                    ...
                    # Save best checkpoint based on average val loss
                    ...
                    if args.wandb_log:
                        wandb.log({
                            ...
                            "val/avg_f2": avg_val_f2,
                            "val/avg_accuracy": avg_val_acc,
                            "lr": lr,
                            "best_val_loss": best_val_loss
                        })
...
def main():
    parser = argparse.ArgumentParser(description="Finetune a MEDIUM model (16L, 8H, 1024D) with Accelerate + DataLoader")
    ...
    parser.add_argument('--pretrained_ckpt_path', type=pathlib.Path, required=True,
                        help='Path to pretrained MEDIUM model checkpoint from training script')
    ...
    parser.add_argument('--eval_langs', type=str, nargs='+', default=None,
                    help='Language names for per-language evaluation reporting (e.g., en es hi ur ru pl)')
    parser.add_argument('--eval_lang_datasets', type=str, nargs='+', default=None,
                    help='HF dataset repos for per-language evaluation (one per eval_lang)')
    parser.add_argument('--eval_parent_langs', type=str, nargs='+', default=None,
                    help='Additional languages from parent_dataset for evaluation (e.g., ta ml)')
    ...
    # Model architecture (MEDIUM model - must match training script)
    parser.add_argument('--n_layer', type=int, default=16, help='Number of layers (MEDIUM model: 16)')
    parser.add_argument('--n_head', type=int, default=8, help='Number of heads (MEDIUM model: 8)')
    parser.add_argument('--n_embd', type=int, default=1024, help='Embedding dimension (MEDIUM model: 1024)')
    ...
    # Logging
    parser.add_argument('--wandb_project', type=str, default='ipa_finetuning_medium')
    ...
    print("="*80)
    print("MEDIUM MODEL ACCELERATED FINE-TUNING")
    print("="*80)
    ...
    # Verify architecture matches medium model
    if args.n_layer != 16 or args.n_head != 8 or args.n_embd != 1024:
        raise ValueError(
            f"Medium model must be 16L/8H/1024D, got {args.n_layer}L/{args.n_head}H/{args.n_embd}D"
        )
    ...
    best_checkpoint_path = args.out_dir / f"{args.dataset}-medium-ckpt.pt"
    ...
    if args.eval_only:
        # Load finetuned weights if checkpoint exists
        if best_checkpoint_path.exists():
            print(f"Loading finetuned weights from {best_checkpoint_path}")
            ...
            print(f"  Loaded weights (val_loss={ft_ckpt.get('val_loss', 'N/A'):.4f}, "
                  f"val_acc={ft_ckpt.get('val_accuracy', 'N/A'):.4f}, "
                  f"val_f2={ft_ckpt.get('val_f2', 'N/A'):.4f})")
        else:
            print("WARNING: No finetuned checkpoint found, evaluating pretrained weights only")
        ...
        if multi_lang:
            ...
            losses = evaluate(model, val_loaders["all"], accelerator)
            print(f"Eval-only: val loss {losses['val']:.4f}, val f2 {losses['val_f2']:.4f}, val accuracy {losses['val_accuracy']:.4f}")

        # Save predictions as .npz if --save_preds is set
        if hasattr(args, 'save_preds') and args.save_preds:
            model.eval()
            all_preds_list = []
            all_labels_list = []
            with torch.no_grad():
            ...
            npz_path = args.out_dir / f"{args.dataset}-preds.npz"
            np.savez(npz_path, labels=all_labels_np, preds=all_preds_np)
            print(f"Saved predictions to {npz_path} ({len(all_labels_np)} samples)")
        return
    ...
    if args.resume_checkpoint is not None and args.resume_checkpoint.exists():
        ...
        best_val_loss = min(best_val_loss, resume_ckpt.get('best_val_loss', float('inf')))
    ...
    best_val_loss = finetune(model, train_loader, val_loaders, optimizer, accelerator,
                             best_val_loss, best_checkpoint_path, args,
    ...
    if multi_lang:
        ...
        total_test_acc = 0.0
        total_test_f2 = 0.0
        lang_names = sorted(test_loaders.keys())
        for lang in lang_names:
            ...
            total_test_acc += test_results['val_accuracy']
            total_test_f2 += test_results['val_f2']

        avg_test_acc = total_test_acc / len(lang_names)
        avg_test_f2 = total_test_f2 / len(lang_names)

        # Write completion marker
        completion_marker = pathlib.Path(args.out_dir) / f"{args.dataset}-COMPLETED"
        marker_lines = [f"Training completed at {time.strftime('%Y-%m-%d %H:%M:%S')}"]
        marker_lines.append(f"Best val loss: {best_val_loss:.4f}")
        for lang in lang_names:
            r = evaluate(model, test_loaders[lang], accelerator)
            marker_lines.append(f"  [{lang}] acc={r['val_accuracy']:.4f} f2={r['val_f2']:.4f}")
        marker_lines.append(f"  [AVG] acc={avg_test_acc:.4f} f2={avg_test_f2:.4f}")
        completion_marker.write_text("\n".join(marker_lines) + "\n")

        print("="*80)
        print(f"Training complete! Results (single best checkpoint):")
        for lang in lang_names:
            r = evaluate(model, test_loaders[lang], accelerator)
            print(f"  [{lang}] acc={r['val_accuracy']:.4f} f2={r['val_f2']:.4f}")
        ...
        # Single combined test evaluation (legacy behavior)
