...
# Dataset class — proper DataLoader pipeline (matches model.py)
...
class GPTClassificationSmall(nn.Module):
    ...
    def prepare_if_needed(self, train_dataset, val_dataset, force_tokenization=False):
        ...
        # Also keep legacy numpy arrays for backward compat with get_batch/estimate_loss
        self.train_data = self.train_dataset.data
        self.train_labels = self.train_dataset.labels
        self.val_data = self.val_dataset.data
