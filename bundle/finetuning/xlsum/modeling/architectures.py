...
def autoregressive_cross_entropy(logits: Tensor, labels: Tensor) -> tuple[Tensor, Tensor, Tensor]:
    """Score next-token prediction rather than the current token at the same position."""
    shift_logits = logits[:, :-1, :].contiguous()
    shift_labels = labels[:, 1:].contiguous()
    num_tokens = (shift_labels != -100).sum()
    if num_tokens.item() == 0:
        zero = logits.new_zeros(())
        return zero, zero, num_tokens
    ...
    return loss, sum_loss, num_tokens
...
class GPTQKVOGenerative(nn.Module):
    ...
    def forward(self, input_ids: Tensor, labels: Tensor | None = None) -> dict[str, Tensor]:
        ...
        loss, sum_loss, num_tokens = autoregressive_cross_entropy(logits, labels)
...
def infer_arch_from_state(state: dict[str, Tensor]) -> dict[str, Any]:
    ...
    known_heads = {
        (12, 768): 6,           # small
        (16, 1024): 8,          # medium
        (20, 1280): 10,         # large_standard
        (24, 1536): 12,         # xl
        (24, 1152): 18,         # legacy old-large (kept for back-compat with prior ckpts)
        (36, 1280): 20,         # legacy
    }
