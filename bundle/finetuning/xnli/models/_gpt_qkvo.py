...
class GPTClassification(nn.Module):
    ...
    def get_batch(self, split):
        ...
        input_ids_batch_np = input_ids[indices]
        lengths = np.count_nonzero(input_ids_batch_np != self.pad_token_id, axis=1)
        max_len = int(lengths.max()) if lengths.size else 0
        max_len = max(max_len, 1)
        input_ids_batch = torch.from_numpy(input_ids_batch_np[:, :max_len]).to(self.device, dtype=torch.int32)
