#!/usr/bin/env bash

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
SRC_ROOT="${REPO_ROOT}/src"

MODE="${1:-ipa}"
case "${MODE}" in
  ipa)
    MODE_CONFIG="transcription/p2g/config/ipa_tokenize.json"
    ;;
  roman)
    MODE_CONFIG="transcription/p2g/config/roman_tokenize.json"
    ;;
  *)
    echo "usage: $(basename "$0") [ipa|roman|debug]" >&2
    exit 1
    ;;
esac

CONFIG_ARGS=("${MODE_CONFIG}")
LOCAL_CONFIG="transcription/p2g/config/local_tokenize.json"
if [[ -f "${SRC_ROOT}/${LOCAL_CONFIG}" ]]; then
  echo "using local config"
  CONFIG_ARGS+=("${LOCAL_CONFIG}")
else
  echo "no local config found"
fi

cd "${SRC_ROOT}"
python -m transcription.p2g.tokenizing \
  "transcription/p2g/config/default_tokenize.json" \
  "${CONFIG_ARGS[@]}"
