#!/bin/bash
#SBATCH --job-name=p2g_training_roman
...
cp "scripts/jobs/cardinal/p2g_local_train_roman.json" "src/transcription/p2g/config/local_train.json"
if [ $? -ne 0 ]; then
        echo "failed to copy local file"
        exit 1
fi
echo "starting script"
./scripts/train_p2g_model.sh roman
