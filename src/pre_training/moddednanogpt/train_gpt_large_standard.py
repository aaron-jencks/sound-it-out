...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int, eot_token: int = 2):
        super().__init__()
    ...
    def create_blockmasks(self, input_seq: Tensor, sliding_window_num_blocks: Tensor):
        BLOCK_SIZE = 128
        docs = (input_seq == self.eot_token).cumsum(0)
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        # Repeat the 16-layer long/short SWA pattern to match the configured layer count.
        base_block_masks = [
        ...
        repeats = (len(self.blocks) + len(base_block_masks) - 1) // len(base_block_masks)
        block_masks = (base_block_masks * repeats)[:len(self.blocks)]
...
def _load_data_shard(file: Path):
    ...
    if version == 1:
        token_dtype = torch.uint16
        bytes_per_token = 2
    else:
        token_bits = int(header[3]) or 32
        if token_bits == 16:
            token_dtype = torch.uint16
            bytes_per_token = 2
        elif token_bits == 32:
            ...
            raise ValueError(f"Unsupported token dtype bits: {token_bits}")
    with file.open("rb", buffering=0) as f:
        tokens = torch.empty(num_tokens, dtype=token_dtype, pin_memory=True) # avoid pin_memory copy by @YouJiacheng
        f.seek(256 * 4)
        nbytes = f.readinto(tokens.numpy()) # avoid bytes->array copy by @YouJiacheng
        assert nbytes == bytes_per_token * num_tokens, "number of tokens read does not match header"
    return tokens
...
class Hyperparameters:
    ...
    eot_token: int = 2 # end-of-text token id used to detect document boundaries
...
def parse_args():
    ...
    parser.add_argument("--eot_token", type=int, default=None,
                        help="End-of-text token ID used to detect document boundaries (must match your tokenizer)")
...
if cli_args.eot_token is not None:
    args.eot_token = cli_args.eot_token

run_id = int(os.environ.get("RUN_ID", 0))
# torchrun sets these env variables
rank = int(os.environ["RANK"])
world_size = int(os.environ["WORLD_SIZE"])
assert world_size in (4, 8), f"expected WORLD_SIZE in (4, 8), got {world_size}"
...
print0(f"  eot_token: {args.eot_token}")
print0("="*100)
...
model: nn.Module = GPT(vocab_size=args.vocab_size, num_layers=20, num_heads=10, model_dim=1280,
                       max_seq_len=max(args.train_seq_len, args.val_seq_len), eot_token=args.eot_token).cuda()
