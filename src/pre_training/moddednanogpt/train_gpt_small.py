...
class Muon(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        # Must wait for all async all_gather calls to complete before returning;
        # otherwise the next forward pass may read stale parameter values.
...
class DistAdam(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        # Must wait for all async all_gather_into_tensor calls to complete before returning;
        # otherwise the next forward pass may read stale parameter values.
        torch.futures.collect_all(futures).wait()
...
class Hyperparameters:
    ...
    eot_token = 2 # end-of-text token id used to detect document boundaries
    # optimization
...
model: nn.Module = GPT(vocab_size=args.vocab_size, num_layers=12, num_heads=6, model_dim=768, max_seq_len=max(args.seq_len, args.val_seq_len), eot_token=args.eot_token).cuda()
